...
}, fdom.apis.register("core.websocket", WS), /*globals OAuth, Promise,chrome,console */
function() {
    "use strict";
    function reqListener(req) {
        var i;
        for (i = 0; i < oAuthFlows.length; i += 1) if (req.url.indexOf(oAuthFlows[i].state) >= 0) {
            oAuthFlows[i].instance.dispatchEvent("oAuthEvent", req.url), oAuthFlows.splice(i, 1);
            break;
        }
        chrome.tabs.remove(req.tabId), chrome.webRequest.onBeforeRequest.removeListener(reqListener);
    }
    function monitorNav(url, inst) {
        var state = Math.random();
        return oAuthFlows.push({
            state: state,
            url: url,
            instance: inst
   ...
   */
    "undefined" != typeof chrome && chrome.permissions.getAll(function(permissions) {
        // Require webRequest permissions.
        permissions.permissions.indexOf("webRequest") < 0 || OAuth.register(function(redirectURIs, instance) {
            var i, j;
            for (i = 0; i < redirectURIs.length; i += 1) for (j = 0; j < permissions.origins.length; j += 1) if (0 === redirectURIs[i].indexOf(permissions.origins[j])) return Promise.resolve({
...
var Storage_chrome = function(channel, dispatch) {
    this.dispatchEvents = dispatch, this.channel = channel;
};
...
Storage_chrome.prototype.keys = function(continuation) {
    chrome.storage.local.get(null, function(data) {
        var item, keys = [];
        for (item in data) data.hasOwnProperty(item) && keys.push(item);
        continuation(keys);
    });
}, /**
...
Storage_chrome.prototype.get = function(key, continuation) {
    try {
        ...
        {
            chrome.storage.local.get(key, function(k, cb, items) {
                cb(items[k]);
...
Storage_chrome.prototype.set = function(key, value, continuation) {
    ...
    var diff = {};
    diff[key] = value, chrome.storage.local.set(diff, continuation);
}, /**
...
"undefined" != typeof fdom && fdom.apis.register("core.storage", Storage_chrome);
...
var Socket_chrome = function(channel, dispatchEvent, id) {
    this.dispatchEvent = dispatchEvent, this.id = id || void 0, this.id && this.startReadLoop();
};
...
Socket_chrome.prototype.getInfo = function(continuation) {
    this.id ? chrome.socket.getInfo(this.id, continuation) : continuation({
        connected: !1
    });
}, /**
...
Socket_chrome.prototype.connect = function(hostname, port, cb) {
    return this.id ? void cb(void 0, {
        errcode: "ALREADY_CONNECTED",
        message: "Cannot Connect Existing Socket"
    }) : void chrome.socket.create("tcp", {}, function(createInfo) {
        this.id = createInfo.socketId, chrome.socket.connect(this.id, hostname, port, function(result) {
            0 > result ? cb(void 0, {
                errcode: "CONNECTION_FAILED",
                message: "Chrome Connection Failed: " + Socket_chrome.ERROR_MAP[result]
            }) : cb(), this.startReadLoop();
        }.bind(this));
    }.bind(this));
}, /**
...
Socket_chrome.prototype.write = function(data, cb) {
    return this.id ? void chrome.socket.write(this.id, data, cb) : void cb(void 0, {
...
Socket_chrome.errorStringOfCode = function(code) {
    return Socket_chrome.ERROR_MAP[String(code)] || "UNKNOWN";
}, /*
...
Socket_chrome.prototype.dispatchDisconnect = function(code) {
    0 === code ? this.dispatchEvent("onDisconnect", {
        errcode: "SUCCESS",
        message: "Socket closed by call to close"
    }) : -100 === code ? this.dispatchEvent("onDisconnect", {
...
Socket_chrome.prototype.handleReadData = function(readInfo) {
    if (readInfo.resultCode <= 0) // Short circuit the read loop.
    throw this.dispatchDisconnect(readInfo.resultCode), new Error("Disconnected");
...
Socket_chrome.prototype.startReadLoop = function() {
    var loop = function() {
        return this.makeSocketReadPromise().then(this.handleReadData.bind(this)).then(loop);
    }.bind(this);
    loop();
}, /**
...
Socket_chrome.prototype.listen = function(address, port, callback) {
    return this.id ? void callback(void 0, {
        errcode: "ALREADY_CONNECTED",
        message: "Cannot Listen on existing socket."
    }) : void chrome.socket.create("tcp", {}, function(createInfo) {
        this.id = createInfo.socketId, // See https://developer.chrome.com/apps/socket#method-listen
        chrome.socket.listen(this.id, address, port, // TODO: find out what the default is, and what this really means, the
        ...
        // to open)
        100, this.startAcceptLoop.bind(this, callback));
    }.bind(this));
}, /**
...
Socket_chrome.prototype.startAcceptLoop = function(callbackFromListen, result) {
    var errorMsg;
    return 0 !== result ? (errorMsg = Socket_chrome.ERROR_MAP.hasOwnProperty(result) ? "Chrome Listen failed: " + Socket_chrome.ERROR_MAP[result] : "Chrome Listen failed: Unknown code " + result, 
    void callbackFromListen(void 0, {
        errcode: "CONNECTION_FAILURE",
        message: errorMsg
    })) : (callbackFromListen(), void chrome.socket.accept(this.id, this.acceptLoop.bind(this)));
}, /**
...
Socket_chrome.prototype.acceptLoop = function(acceptInfo) {
    ...
    return this.id && chrome.socket.accept(this.id, this.acceptLoop.bind(this)), 0 !== acceptInfo.resultCode ? void console.warn("Failed to accept " + this.id + ": " + acceptInfo.resultCode) : void chrome.socket.getInfo(acceptInfo.socketId, function(info) {
...
Socket_chrome.prototype.close = function(continuation) {
    this.id ? (chrome.socket.disconnect(this.id), chrome.socket.destroy(this.id), delete this.id, 
    continuation()) : continuation(void 0, {
        errcode: "SOCKET_CLOSED",
        message: "Socket Already Closed"
    });
}, /** REGISTER PROVIDER **/
"undefined" != typeof fdom && fdom.apis.register("core.tcpsocket", Socket_chrome);
...
var UdpSocket_chrome = function(channel, dispatchEvent) {
    this.dispatchEvent = dispatchEvent, this.id = void 0;
};
...
UdpSocket_chrome.prototype.bind = function(address, port, continuation) {
    chrome.socket.create("udp", {}, function(createResult) {
        this.id = createResult.socketId, chrome.socket.bind(this.id, address, port, function(bindResult) {
            bindResult >= 0 ? (continuation(bindResult), this.read()) : continuation(void 0, {
                errcode: "BIND_FAILED",
...
UdpSocket_chrome.prototype.read = function() {
    chrome.socket.recvFrom(this.id, null, function(recvFromInfo) {
        return recvFromInfo.resultCode < 0 ? void console.warn("Failed to read from " + this.id + ": " + recvFromInfo.resultCode) : (this.dispatchEvent("onData", {
...
UdpSocket_chrome.prototype.sendTo = function(data, address, port, cb) {
    return this.id ? void chrome.socket.sendTo(this.id, data, address, port, function(writeInfo) {
...
UdpSocket_chrome.prototype.destroy = function(continuation) {
    this.id && "INVALID" !== this.id ? (chrome.socket.destroy(this.id), this.id = "INVALID", 
    continuation()) : continuation(void 0, {
        errcode: "SOCKET_CLOSED",
        message: "Socket Already Closed"
    });
}, /** REGISTER PROVIDER **/
"undefined" != typeof fdom && fdom.apis.register("core.udpsocket", UdpSocket_chrome);
