...
function SimpleDataPeer(peerName, stunServers, dataChannelCallbacks, mocks) {
    ...
    "undefined" != typeof mocks.RTCPeerConnection) this.RTCPeerConnection = mocks.RTCPeerConnection; else if ("undefined" != typeof webkitRTCPeerConnection) this.RTCPeerConnection = webkitRTCPeerConnection; else {
...
var Socket_chrome = function(channel, dispatchEvent, id) {
    console.warn(id), this.dispatchEvent = dispatchEvent, this.id = id || void 0, this.id && this.read();
};
...
Socket_chrome.prototype.read = function() {
    var loop;
    loop = function() {
        return this.doRead().then(this.checkReadResult.bind(this)).then(function(data) {
            this.dispatchEvent("onData", {
                data: data
            });
        }.bind(this)).then(loop);
    }.bind(this), loop().then(null, function(err) {
        console.warn("Read Error [" + this.id + "]: " + err.message), this.dispatchEvent("onDisconnect", {
            errcode: "READ_ERROR",
            message: err.message
        });
    }.bind(this));
...
Socket_chrome.prototype.checkReadResult = function(readInfo) {
    ...
    return 0 === code ? Promise.reject(new Error("remotely closed.")) : 0 > code ? Promise.reject(new Error(Socket_chrome.ERROR_MAP[String(code)] || code)) : Promise.resolve(readInfo.data);
}, /**
...
Socket_chrome.prototype.listen = function(address, port, callback) {
    return this.id ? void callback(void 0, {
        ...
        this.id = createInfo.socketId, chrome.socket.listen(this.id, address, port, null, this.accept.bind(this, callback));
    }.bind(this));
}, Socket_chrome.prototype.accept = function(callback, result) {
    var acceptCallback;
    // Begin accepting on the listening socket.
    return 0 !== result ? void callback(void 0, {
        errcode: "CONNECTION_FAILURE",
        message: "Chrome Listen failed: " + Socket_chrome.ERROR_MAP[result]
    }) : (callback(), acceptCallback = function(acceptInfo) {
        0 === acceptInfo.resultCode ? (chrome.socket.getInfo(acceptInfo.socketId, function(info) {
            this.dispatchEvent("onConnection", {
                socket: acceptInfo.socketId,
                host: info.peerAddress,
                port: info.peerPort
            });
        }.bind(this)), chrome.socket.accept(this.id, acceptCallback)) : -15 !== acceptInfo.resultCode ? console.warn("Failed to accept " + this.id + ": " + acceptInfo.resultCode) : chrome.socket.accept(this.id, acceptCallback);
    }.bind(this), void chrome.socket.accept(this.id, acceptCallback));
}, /**
